...
packer {
  required_plugins {
    vsphere = {
      ...
      version = ">= 1.2.5"  // 1.4.2_x5.0_darwin_arm64 (2025-04-11)
    }
...
locals {
  ...
  timestamp 		     = formatdate("YYYYMMDD", timeadd(timestamp(), "60m"))
  vm_name                    = "Windows2022-${local.timestamp}"
...
source "vsphere-iso" "WIN2022_Std" {
  ...
  cd_files = [
    "./windows/scripts/windows-init.ps1",
    "./windows/scripts/windows-prepare.ps1",
    "./windows/scripts/windows-vmtools.ps1",
  ]
  cd_content = {
    "autounattend.xml" = templatefile("${abspath(path.root)}/data/autounattend.pkrtpl.hcl", {
      build_username       = "Administrator"
      build_password       = "<admin_password>"
